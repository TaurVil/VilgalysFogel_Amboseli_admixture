...
#########   Chunk to get data       ###########
load(paste("./windows.",d_name,".RData", sep=""))

#########   Recombination rate result       ###########
t <- cor.test(to_analyze$mean_ancestry,to_analyze$recombination,method=c('spearman')); t; t$p.value; rm(t)
t <- cor.test(to_analyze$recent_ancestry,to_analyze$recombination,method=c('spearman')); t; t$p.value; rm(t)
t <- cor.test(to_analyze$historical_ancestry,to_analyze$recombination,method=c('spearman')); t; t$p.value; rm(t)
