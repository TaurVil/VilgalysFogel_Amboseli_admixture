...
# Load ancestry in 250 kb windows for all Amboseli individuals available in the Duke Data Repository at XXX
ancestry <- read.table("amboseli.ancestry.250kwin.txt", header=T)
...
for (i in 1:length(years_list)) {
  ...
  which(colnames(ancestry) %in% tmp$table_s1_id) -> k
  
  tmp2 <- ancestry[c(1:3,k)]
...
ancestry2 <- ancestry %>% select(chr, start, end)
...
# 1. FST - to get these estimates, use the script 4bFST_SNPRCref.sh in "VilgalysFogel_Amboseli_admixture/02_Local ancestry calling/02.3.pedigree_trios/" but substituting 250000 for window size --fst-window-size in place of 35000.
fst <- read.table("fst_masked_unmerged_SWref_250kbwin_500bpstep.windowed.weir.fst", header=T) 
...
# 2. Mean recombination rate and B-values - get these from ******XXX TAURASSSSS FILE***** which has already removed windows in which recombination rate estimates were greater than 100x larger than the median recombination rate 
rcr_bvalues <- to_analyze # ******ARIELLE - TAURAS FILE HERE
# Grab chromosome, start end, rcr, and B values
rcr_bvalues <- rcr_bvalues[c(1:4, 22)]
# Get the center position for each 250 kb window
rcr_bvalues$pos <- ((rcr_bvalues$end-rcr_bvalues$start)/2)+rcr_bvalues$start
# Give each site a unique id
rcr_bvalues$id <- paste(rcr_bvalues$chr, rcr_bvalues$pos, sep="_")
# Merge rcr and B-value estimates with ancestry_change+FST data frame
all_data <- merge(tmp, rcr_bvalues, by=c("id", "chr", "start", "end", "pos"))

summary(lm(annual_anubis_change_beta~starting_pop_avg_anubis_admix_1979+rcr+WEIGHTED_FST+B, data=all_data))

write.csv(summary(lm(annual_anubis_change_beta~starting_pop_avg_anubis_admix_1979+rcr+WEIGHTED_FST+B, data=all_data))$coef, file="tableS6_results.csv")
