...
## load in recombination rates. This is the `recombination` column produced in Section 05
load("../../05_selection-vs-introgression/windows.25kb.RData")
## removes windows with unreasonable recombination
subset(features, features$recombination < 100*median(features$recombination)) -> res2
s3 <- NULL 
...
for (i in unique(snp$V2)) {
	...
	r1$sum <- cumsum(r1$recombination)
