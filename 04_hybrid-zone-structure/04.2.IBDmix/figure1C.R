...
mikumi_SNPRCyellow_ibd <- read.table("./RESULTS/ibdmix_yellow_estimates.txt", header=T) 
# Load IBDmix results for high coverage Amboseli individuals
amboseli_ibd <- read.table("./RESULTS/ibdmix_amboseli_estimates.txt", header=T) 
...
set.seed(1234)
ggplot(data=ibd2[ibd2$species!="aberdares",])  + geom_jitter(aes(x=population, y=ancestry, color=species), alpha=0.8, width=0.05, size=2, stroke=1) + scale_fill_manual(values=c("#009E73","#4FC7FF",  "mediumorchid3"), labels=c("anubis", "hamadryas", "guinea")) + scale_color_manual(values=c("#009E73","#4FC7FF", "mediumorchid3"), labels=c("anubis",  "hamadryas", "guinea")) + theme_classic() + theme(text=element_text(size=25), axis.text = element_text(color="black")) + scale_y_continuous("proportion identical-by-descent")
