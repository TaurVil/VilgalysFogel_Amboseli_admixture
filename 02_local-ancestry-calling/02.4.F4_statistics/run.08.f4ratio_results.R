...
# Load the R data results for each chromosome generated by the script run.07.f4ratio_admixr.R and concatenate into a single data frame called f4
# These Rdata files are available in DATA and are named f4_ratio_CHROMOSOME.Rd
for (i in seq(1,19)) {
...
# Plot the results (alpha +/- 2 SE) for all phylogenetic configurations colored by population X (Amboseli historically admixed individuals or Amboseli recently admixed individuals)
ggplot() + geom_pointrange(data=subset(f4, X=="Amboseli_recent"), aes(x=phylogeny_noX, y=alpha, ymin = alpha - (2 * stderr), ymax = alpha + (2 * stderr)), color="#009E73") + geom_pointrange(data=subset(f4, X=="Amboseli_historic"), aes(x=phylogeny_noX, y=alpha, ymin = alpha - (2 * stderr), ymax = alpha + (2 * stderr)), color="#FFED4F") +  geom_hline(yintercept = 0, linetype = 2) + facet_wrap(~chrom) + labs(y = "anubis ancestry proportion", x = "phylogeny (A, B, C, Outgroup)") + theme_classic() + scale_x_discrete(labels=c("Guinea/SNPRC_anubis_founder/Mikumi/Gelada" = "Guinea/SNPRC_anubis_founder/\nMikumi/Gelada",  "Guinea/SNPRC_anubis_founder/Mikumi/macaque" = "Guinea/SNPRC_anubis_founder/\nMikumi/macaque", "Hamadryas/SNPRC_anubis_founder/Mikumi/Gelada" = "Hamadryas/SNPRC_anubis_founder/\nMikumi/Gelada", "Hamadryas/SNPRC_anubis_founder/Mikumi/macaque"="Hamadryas/SNPRC_anubis_founder/\nMikumi/macaque")) + geom_text(data=f4, aes(x=phylogeny_noX, y=alpha, label=Zscore), size=3, nudge_x=0.4) + theme(axis.text.x = element_text(size=6.5))

# Get a weighted average based on chromosome size
# Get base length for chromosomes in the MacaM genome (MacaM.fa.fai is available in DATA)
...
# Multiply the average alpha across phylogenetic configurations per chromosome by the chromosome length
