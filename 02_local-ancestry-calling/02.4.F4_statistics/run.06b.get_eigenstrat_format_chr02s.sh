...
vcftools --gzvcf final.baboon.to.macam.n49.CHROM.rename.vcf.gz --mac 1 --plink --out CHROM.n49 # only difference between this script and run.06a.get_eigenstrat_format_excluding_chr02s.sh is using the renamed vcf file here for chromosomes 02a and 02b
# mac 1 = only include sites with a minor allele count of at least 1 (i.e., all samples are not all carrying the major allele) - this filtering is not really necessary because sites where all samples only have the major allele won’t be informative for f-statistics
...
# Finally, convert plink format to eigenstrat format using convertf program from EIGENSOFT
