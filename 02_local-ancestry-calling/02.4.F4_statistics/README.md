...
Obtain fastq files for high coverage individuals from Amboseli and unadmixed populations, then map these data to the rhesus macaque genome (MacaM; Zimin et al. 2014 Biology Direct) using bowtie2. Some of these files are split between multiple SRA ascensions, so they will need to be merged together into a single file before we can proceed. 
...
for f in `cat XXXOnce you get SRR numbersXXX`; do sed -e s/NAME/$f/g run.01a.bowtie2_map_macam.SRRs1.sh >> run.$f.sh; sbatch --mem=32000 --out=mapping.$f.out run.$f.sh; rm run.$f.sh; done # for high coverage Amboseli data (excluding data from NCBI SRA BioProject PRJNA295782) and Mikumi data where paired-end reads are labeled as R1 and R2

cat SRR_list_BGDP SRR_list_SNPRC_amboseli_published SRR_list_SNPRC_anubis_founders >> SRR_tmp
for f in `cat SRR_tmp`; do sed -e s/NAME/$f/g run.01a.bowtie2_map_macam.SRRs2.sh >> run.$f.sh; sbatch --mem=32000 --out=mapping.$f.out run.$f.sh; rm run.$f.sh; done # for all other data where paired-end reads are labeled as _1 and _2
...
For each individual, call genotypes using GATK (gVCF pipeline) and then merge those genotypes into a single file. 
...
# to use the macaque as a possible outgroup, we add the macaque genotype (homozygous reference) as an additional sample at all sites
...
Having completed joint genotype calling, we next needed to format each vcf into EIGENSTRAT format. We then calculate F4 statistics using admixr (Petr et al. 2019 Bioinformatics; https://github.com/bodkan/admixr)
