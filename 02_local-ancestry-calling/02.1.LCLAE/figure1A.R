...
# Plot ancestry tracts for all populations included in this study
# Plot a horizontal row corresponding to the first 20 Mb of chromosome 1 for each individual, organized from top to bottom from the most anubis to most yellow

# 1. Amboseli local ancestry tracts
# Load ancestry tracts for Amboseli individuals generated using masked SNPRC individuals for the reference panel (used in main text analyses) available in the Duke Data Repository at XXX
...
# Calculate genome-wide anubis ancestry so we can organize individuals from the most anubis to the most yellow (this produces the same estimates as in Table S1)
overall_admix <- tracts %>% group_by(table_s1_id, state) %>% summarize(total_state_length=sum(length2), .groups="keep") %>% as.data.frame()
overall_admix2 <- overall_admix %>% group_by(table_s1_id) %>% mutate(total_length=sum(total_state_length)) %>% as.data.frame() #weird here - but I think I've fixed it
