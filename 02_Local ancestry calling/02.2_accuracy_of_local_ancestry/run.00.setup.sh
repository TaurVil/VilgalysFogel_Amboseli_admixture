...
## 
# get yellow and anubis allele frequencies, from SNPRC yellow and anubis individuals (the same reference panel used for LCLAE)
...
# these are saved in a_priori_genotypes, for use during the script run.02.create_sample_vcf.R
...
## get header to be added to the vcf files after genotypes are simulated. This is added within run.03.get_fasta.sh, and can come from any vcf file. 
zcat ~/panubis1_genotypes/calls_merged/04.merged_shared.1.vcf.gz | grep '^#' | sed '$ d' > full_header.vcf 

## create last line of vcf header to which genotype information will be appended during run.02.create_sample_vcf.R
