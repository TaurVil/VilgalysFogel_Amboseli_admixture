...
#############################################################################################################################
# Ancestry state inconsistencies across pedigree trios.
#############################################################################################################################
...
#############################################################################################################################
# Covariates to include in model predicting a site's propotion of ancestry state inconsistencies across pedigree trios.
#############################################################################################################################
...
# Mostly repeat lines 30-122 for the Wall et al. data (note that we'll use the same rcr calls so we can just merge the Wall et al. data with the rcr data frames)
...
# Are some predigree trios more or less inconsistent than others?
...
# Minimum coverage seems to be the most important factor for predicting ancestry state inconsistencies in a pedigree trio
# Plot minimum coverage in a trio (x-axis) vs. proportion of pedigree inconsistencies per trio (y-axis)
ggplot(data=trios_inconsistencies)  + geom_smooth(aes(min_cov, prop_inconsistent), method="lm", color="mediumpurple3") +  scale_x_continuous(name="minimum genome-wide coverage in the trio", breaks=seq(0,1.4,by=0.2)) + geom_point(aes(min_cov, prop_inconsistent), size=4, alpha=0.7, stroke=1.5, color="grey50") + scale_y_continuous(name="a pedigree trio’s\nproportion of ancestry inconsistencies")  + theme_classic() + theme(text=element_text(size=20), axis.text = element_text(color="black")) 
