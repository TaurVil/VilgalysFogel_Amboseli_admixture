...
ggplot(gene) + 
  ...
  theme_classic() + theme(legend.position = "none", axis.text = element_text(colour = "black"), text=element_text(size=20, family="Helvetica")) -> a
...
ggplot(data=reduction, aes(x=as.factor(qqq), y=t_data, group=qqq)) +
  ...
  theme_classic() +  theme(text=element_text(size=20, family="Helvetica"), axis.text = element_text(color="black"), axis.text.x = element_text(face="italic", size=12, family="Helvetica"), axis.title.x=element_text(vjust=-1))  -> b 
...
ggplot(data=results) +
  ...
  scale_x_continuous(name="proportion of genes", labels=c("0.1" = "top 10%\nvs.\nbottom 10%", "0.15" = "15%", "0.2" = "top 20%\nvs.\nbottom 20%", "0.25" = "25%","0.3" = "top 30%\nvs.\nbottom 30%", "0.35" = "35%","0.4" = "top 40%\nvs.\nbottom 40%"), breaks=seq(0.1,0.4, 0.05)) + scale_y_continuous("Spearman's rho", limits=c(0, 0.2)) +  theme_classic() +  theme(text=element_text(size=20, family="Helvetica"), axis.text = element_text(color="black"), axis.text.x = element_text(face="italic", size=12, family="Helvetica"), axis.title.x=element_text(vjust=-1)) + annotate("text", x = 0.15, y = 0.18, label = "*", size=8) +  annotate("text", x = 0.2, y = 0.18, label = "*", size=8) -> c
