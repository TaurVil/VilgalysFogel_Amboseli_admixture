...
(((plotDleft + theme(plot.margin = unit(c(0,0,25,0), "pt"))) | plotDright) + xlab(label = "number of fixed differences per Mb") + theme(axis.title.x = element_text( hjust=3.5, vjust=-0.3))) / (((plotEleft + theme(plot.margin = unit(c(0,0,25,0), "pt"))) | plotEright) + xlab(label = "rank-ordered B values") + theme(axis.title.x = element_text(hjust=-2.5,vjust=-0.3))) / (((plotFleft + theme(plot.margin = unit(c(0,0,25,0), "pt"))) | plotFright) + xlab(label = "rank-ordered recombination rate") + theme(axis.title.x = element_text(hjust=5.5, vjust=-0.3)))

ggsave("fig3DEF.png") # this plot, I manually export as image and save as "png" with 1100 width x 1900 height
